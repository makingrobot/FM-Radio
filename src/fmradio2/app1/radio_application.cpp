...
void RadioApplication::OnInit() {
    ...
    board->GetAdcDriver()->Init(I2S_NUM_0, 96000, I2S_DATA_BIT_WIDTH_24BIT);
    board->GetDacDriver()->Init(I2S_NUM_1, 96000, I2S_DATA_BIT_WIDTH_24BIT);
    ...
    xTaskCreate([](void* pvParam){
        RadioApplication *app = (RadioApplication*)pvParam;
        app->AudioOutputTask();
    }, "audio_output_task", 4096, this, 5, &audio_output_taskhandle_);
}
...
void RadioApplication::AudioInputTask() {
    ...
    while (1)
    {
        ...
        if (ret == ESP_OK)
        {
            Log::Warn(TAG, "Read Data: %d", audio_data.size);
